...
private:
    ...
    // Private Constructor: client cannot instantiate AsteroidContext directly.
    // AsteroidContextBuilder will build it.
...
public:
    // AsteroidContextBuilder is defined separately outside and needs access to the private constructor
    friend class AsteroidContextBuilder;
...
// ============================================================================
//  4. BUILDER WITH FLYWEIGHT INTEGRATION — AsteroidContextBuilder
// ----------------------------------------------------------------------------
//  Ye Builder ek master-stroke hai! Client ko details chupani hain ki hum 
//  Flyweight use kar rahe hain ya nahi. Client bas saari fields (intrinsic & extrinsic) 
//  builder ko batayega aur build() call karega. 
//  Internally, Builder class:
//    1. Intrinsic values ke corresponding factory se shared flyweight lo.
//    2. Halka Context object banao aur return karo.
// ============================================================================
class AsteroidContextBuilder {
private:
    // Intrinsic fields staging variables (Flyweight Factory me lookup karne ke liye)
    int length = 0;
    int width = 0;
    int weight = 0;
    string color;
    ...
    // Extrinsic fields staging variables (Context objects ke personal values)
    int posX = 0;
    int posY = 0;
    int velocityX = 0;
    int velocityY = 0;
...
public:
    // Fluent interface setters returning Builder reference for method chaining
    AsteroidContextBuilder& setDimensions(int l, int w) {
        this->length = l;
        this->width = w;
        return *this;
    }

    AsteroidContextBuilder& setWeight(int wt) {
        this->weight = wt;
        return *this;
    }

    AsteroidContextBuilder& setColor(const string& col) {
        this->color = col;
        return *this;
    }

    AsteroidContextBuilder& setTexture(const string& tex) {
        this->texture = tex;
        return *this;
    }

    AsteroidContextBuilder& setMaterial(const string& mat) {
        this->material = mat;
        return *this;
    }

    AsteroidContextBuilder& setPosition(int x, int y) {
        this->posX = x;
        this->posY = y;
        return *this;
    }

    AsteroidContextBuilder& setVelocity(int vx, int vy) {
        this->velocityX = vx;
        this->velocityY = vy;
        return *this;
    }

    // Build process: factory se flyweight fetch karke context ke sath merge karta hai.
    AsteroidContext* build() const {
        // Intrinsic state ke zariye factory se cache-checked flyweight pointer fetch karo
        AsteroidFlyweight* fw = AsteroidFactory::getAsteroid(length, width, weight, color, texture, material);
        
        // Halka AsteroidContext object return karo jiske paas apna extrinsic state 
        // aur heavy intrinsic state ka shared reference (pointer) ho.
        return new AsteroidContext(fw, posX, posY, velocityX, velocityY);
    }
};
...
public:
    void spawnAsteroids(int count) {
        ...
        for (int i = 0; i < count; i++) {
            ...
            // CLIENT/SPACEGAME KE LIYE KAMAAL:
            // Builder use karne se Client ko ye tension nahi leni ki intrinsic state 
            // factory se kaise laayein aur context kaise connect karein. 
            // Builder internally Flyweight Factory se shared pointer fetch karega
            // aur Context create kar dega. Pure Fluent API!
            AsteroidContext* context = AsteroidContextBuilder()
