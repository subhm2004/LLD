...
public:
    ...
    //    Constructor 4 baar `new` karta hai, par delete kabhi nahi hota.
    //    Hona chahiye tha:
    //        ~VendingMachine() {
    //            delete noCoinState;  delete hasCoinState;
    //            delete dispenseState; delete soldOutState;
    //        }
    //    (Aur uske liye VendingState ka destructor virtual hona ZAROORI hai —
    //     upar wala point yaad karo. Dono bugs jude hue hain.)
    //
    //    Sabse saaf fix: raw pointer ki jagah `unique_ptr<VendingState>` use karo
    //    — phir delete apne aap ho jaata hai aur bhoolne ka sawaal hi nahi.
};
