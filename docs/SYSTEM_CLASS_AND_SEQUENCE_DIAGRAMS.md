...
**Folder:** [`GPay_LLD/`](../GPay_LLD/) · **Facade:** `GPaySystem` · **Patterns:** Strategy (payment rail), Factory, Service layer
...
classDiagram
    ...
    class BankAccountRailStrategy
    class WalletRailStrategy
    ...
    GPaySystem --> TransferService
    GPaySystem --> PinAuthService
    GPaySystem --> DailyLimitService
    GPaySystem --> TransactionLedgerService
    GPaySystem --> RequestMoneyService
    TransferService --> TransactionLedgerService
    TransferService --> DailyLimitService
    TransferService --> PaymentRailFactory
    TransferService --> TransactionFactory
    PaymentRailFactory ..> IPaymentRailStrategy
    IPaymentRailStrategy <|-- BankAccountRailStrategy
    IPaymentRailStrategy <|-- WalletRailStrategy
    GPaySystem --> User
    GPaySystem --> BankAccount
    GPaySystem --> Wallet
    TransferService --> Transaction
```

### Sequence Diagram — sendMoney (linked bank)

```mermaid
sequenceDiagram
    autonumber
    actor Payer
    participant GP as GPaySystem
    participant Pin as PinAuthService
    participant TS as TransferService
    participant Lim as DailyLimitService
    participant Rail as BankAccountRailStrategy
    participant Led as TransactionLedgerService

    Payer->>GP: sendMoney(payerId, payeeUpi, amount, pin, LINKED_BANK)
    GP->>Pin: verifyPin(payer, pin)
    GP->>TS: execute(payer, payee, amount, ...)
    TS->>Lim: recordSpend(payerId, amount)
    TS->>Rail: debit(payerId, amount)
    TS->>Rail: credit(payeeId, amount)
    TS->>Led: save(txn SUCCESS)
    TS-->>GP: Transaction
    GP-->>Payer: SUCCESS txnId
```

### Sequence Diagram — scanAndPay (QR)
...
sequenceDiagram
    ...
    actor User
    participant GP as GPaySystem
    participant Utils as UpiUtils
    participant TS as TransferService

    User->>GP: scanAndPay(payerId, qrPayload, pin, rail)
    GP->>Utils: parseUpiQr(qrPayload)
    Utils-->>GP: payeeUpi, amount, note
    GP->>GP: resolveUserByUpi(payeeUpi)
    GP->>TS: execute(..., QR_PAY, ...)
    TS-->>GP: Transaction
    GP-->>User: SUCCESS
```

### Sequence Diagram — request money + fulfill
...
sequenceDiagram
    ...
    actor Payer
    participant GP as GPaySystem
    participant Req as RequestMoneyService
    participant TS as TransferService

    Merchant->>GP: requestMoney(requesterId, payerUpi, amount)
    GP->>Req: createRequest(...)
    Req-->>GP: requestId
    Payer->>GP: fulfillMoneyRequest(payerId, requestId, pin, rail)
    GP->>TS: execute(..., REQUEST_FULFILL, ...)
    GP->>Req: markFulfilled(requestId)
    GP-->>Payer: Transaction SUCCESS
```
...
**Folder:** [`Truecaller_LLD/`](../Truecaller_LLD/) · **Facade:** `TruecallerSystem` · **Patterns:** Strategy (spam scoring), Service layer
...
classDiagram
    ...
    class DefaultSpamScoringStrategy
    ...
    TruecallerSystem --> LookupService
    TruecallerSystem --> ContactSyncService
    TruecallerSystem --> SpamReportService
    TruecallerSystem --> BlockService
    TruecallerSystem --> SearchService
    TruecallerSystem --> CallLogService
    SpamReportService --> ISpamScoringStrategy
    ISpamScoringStrategy <|-- DefaultSpamScoringStrategy
    TruecallerSystem --> PhoneProfile
    LookupService --> CallerLookupResult
```

### Sequence Diagram — identifyCaller (incoming call)

```mermaid
sequenceDiagram
    autonumber
    actor User
    participant TC as TruecallerSystem
    participant Lk as LookupService
    participant Bl as BlockService
    participant Dir as directory_

    User->>TC: identifyCaller(forUserId, callerPhone)
    TC->>Lk: lookup(forUserId, callerPhone, directory, blockService)
    Lk->>Bl: isBlocked(forUserId, phone)?
    alt blocked
        Lk-->>TC: CallerLookupResult BLOCKED
    else not blocked
        Lk->>Dir: resolve PhoneProfile + synced name
        Lk-->>TC: CallerLookupResult name, tag, spamScore
    end
    TC-->>User: display name / SPAM warning
```

### Sequence Diagram — reportSpam + block
...
sequenceDiagram
    ...
    actor Reporter
    participant TC as TruecallerSystem
    participant Spam as SpamReportService
    participant Strat as DefaultSpamScoringStrategy
    participant Dir as directory_

    Reporter->>TC: reportSpam(userId, phone, reason)
    TC->>Spam: reportSpam(..., directory)
    Spam->>Dir: increment report count
    Spam->>Strat: computeScore(profile)
    Strat-->>Spam: updated spam score
    Reporter->>TC: blockNumber(userId, phone)
    TC->>TC: BlockService.block()
```

### Sequence Diagram — contact sync
...
sequenceDiagram
    ...
    actor User
    participant TC as TruecallerSystem
    participant Sync as ContactSyncService
    participant Dir as directory_

    User->>TC: syncContacts(userId, contactList)
    TC->>Sync: syncContacts(contacts, directory)
    Sync->>Dir: match phones to PhoneProfile
    Sync-->>TC: enriched ContactEntry list
    TC-->>User: synced contacts with names/tags
```
...
graph LR
    ...
    subgraph Strategy
        ...
        Runner[ICodeRunner]
        PayRail[IPaymentRailStrategy]
        SpamScore[ISpamScoringStrategy]
    end
    ...
    GPAY --> PayRail
    PayRail --> BankAccountRailStrategy
    PayRail --> WalletRailStrategy
    TC --> SpamScore
    SpamScore --> DefaultSpamScoringStrategy
```
...
<details>
<summary><strong>§25 GPay</strong> — UPI send + QR · <a href="./GPay_LLD/">code</a></summary>
...
sequenceDiagram
    ...
    participant GP as GPaySystem
    participant TS as TransferService
    Payer->>GP: sendMoney(upi, pin, LINKED_BANK)
    GP->>TS: debit bank + credit payee
    TS-->>Payer: TXN SUCCESS
```

</details>

<details>
<summary><strong>§26 Truecaller</strong> — caller ID + spam · <a href="./Truecaller_LLD/">code</a></summary>
...
sequenceDiagram
    ...
    actor User
    participant TC as TruecallerSystem
    User->>TC: identifyCaller(incomingPhone)
    TC-->>User: name / SPAM / BLOCKED
```
